...
# https://studio.edge.edx.org/container/i4x://DavidsonCollege/DAP002/vertical/3d73127119f848948afbccb7f73dce06
...
tGen = templateGenerator.templateGenerator(course_id='DavidsonCollege/DAP002/3T2014',
                                           Title='Money and the Money Market: LC4 HOA #3',
                                           MacroJS='../../JS/Macro_1Board.js',
                                           JS='MMM_LC4_HOA_3.js',
                                           OutputFile='MMM_LC4_HOA_3.html',
                                           studioPaths=False 
                                          )
...
#     state = json.loads(json.loads(ans)['answer'])
    ...
    print dist1D(state['MS2']['X1'],state['MS1']['X1'])

    ### First Check Y Direction - should be no shift
    if dist1D(state['MS2']['Y1'],state['MS1']['Y1']) < 1.5:
    ...
    elif dist1D(state['MS2']['Y1'],state['MS1']['Y1']) > 2.0:
        return {'ok': False, 'msg': 'Please rethink your solution?'}
    else:
        return {'ok': False, 'msg': 'Please rethink your solution.'}
