var Macro = (function(JXG, MacroLib) {
  ...
  function init() {
    ...
    //Sliders
    var sliderx = brd1.create('slider',[[8.0,-1.0],[8,-1.0],[1.39,1.4,1.4]],{withLabel:false,snapWidth:0.05,
                                                                           color:'White'});
    var slidery = brd1.create('slider',[[-1.0,2.75],[-1.0,8.75],[-1.4,0.0,0.0]],{withLabel:false,snapWidth:0.05,
                                                                           color:'Crimson'});
    ...
    //Supply Line 1 - fixed
    var SRAS1 = MacroLib.createLine(brd1,{'ltype':'Supply','name':'AS<sub>1924</sub>',color:'DodgerBlue'});
    SRAS1.setAttribute({"dash":1,'fixed':true,'highlight':false});

    //Demand Line 2 - moveable
    var SRAS2 = MacroLib.createTransformLine(brd1,{'transformList':[sliderXPositive],'ltype':'Supply','name':'AS<sub>1929</sub>','color':'DodgerBlue'})
    SRAS2.setAttribute({'withLabel':false,'highlight':true,"visible":true});

    //Demand Line 1 - fixed
    var AD1 = MacroLib.createLine(brd1,{'ltype':'Demand','name':'AD','color':'Crimson'})
    ...
    var H2 = MacroLib.createTransformLine(brd1,{'transformList':[sliderYPositive],'ltype':'Horizontal','name':'H','color':'Orange'});
    ...
    var sliderLabel100 = brd1.create('text',
                                     [0.15,
                                      function(y) {return (H2.point1.Y() + 0.3)},
                                      function() {return (102.0 - 2.0*slidery.Value()/slidery._smin ).toFixed(0);}
                                      ], {visible:false});
    ...
    brd1.on('mousedown', function() {
        ...
        sliderLabel100.setAttribute({visible:true});
        brd1.update()
    });
  }
