var Macro = (function(JXG, MacroLib) {
    ...
    function init() {
        ...
        brd1 = MacroLib.createBoard('jxgbox1',{bboxlimits:bbox1,xname:"Quantity Loanable Funds (Public and Private)", 'xpos':[4,-1.1],
                                          yname:"Real<br>Interest<br>Rate",grid:false,'ypos':[-2.2,10.0]});


        //Supply Line 1 - fixed
        var SBfix = MacroLib.createLine(brd1,{'ltype':'Supply','name':'S<sub>LF</sub>',color:'Orange',storkeWidth:4});
        SBfix.setAttribute({'dash':1,'fixed':true, 'withLabel':false});

        //Supply Line 2 - moveable
        var SB1 = MacroLib.createLine(brd1,{'ltype':'Supply','name':'S<sub>LF</sub>',color:'Orange'});
        SB1.setAttribute({'highlight':false,'withLabel':true});

        //Demand Line 1 - fixed
        var DB1 = MacroLib.createLine(brd1,{'ltype':'Demand','name':'D<sub>LF</sub>','color':'DodgerBlue',storkeWidth:4})
        DB1.setAttribute({'dash':1,'fixed':true, 'withLabel':false,'highlight':false});

        //Demand Line 2 - moveable
        var DB1 = MacroLib.createLine(brd1,{'ltype':'Demand','name':'D<sub>LF</sub>','color':'DodgerBlue'})
        ...
        brd2 = MacroLib.createBoard('jxgbox2',{bboxlimits:bbox2,xname:"Private Investment $", 'xpos':[6.0,-1.5],
                                          yname:"Real<br>Interest<br>Rate",grid:false,'ypos':[-2.2,10.0]});

        //Demand Line 2 - moveable
        var DB2 = MacroLib.createLine(brd2,{'ltype':'Demand','name':'Invest<br>Demand</sub>','color':'Crimson'})
    ...
    function getGrade(){ // Was called getInput
